      ...
      <div class="card m-5 mt-0">
          ...
          <div class="card-body">
            <form id="formPost" method="POST" enctype="multipart/form-data">
                  ...
                  <input type="text" name="judul" id="judul" class="form-control" placeholder="Judul" value="<?=$judul?>" required>
                  ...
                  <textarea name="sinopsis" id="sinopsis" class="form-control" placeholder="Sinopsis" required maxlength="800"><?=$sinopsis?></textarea>
                  <label for="konten" class="form-label fs-5 mt-3">Konten</label>
                  <div id="editor" style="height: 200px;"><?=$isi?></div>
                  ...
                  <button type="submit" class="btn btn-primary mt-1 form-control" name="submit">Submit</button>
            </form>
      </section>
...
if(isset($_POST['submit'])){
  ...
  $id = $idbuku;
  if(!empty($foto)){
    $sql = "UPDATE karya SET judul = '$judul', sinopsis = '$sinopsis', tipe = '$tipe', konten = '$konten', gambar = '$foto' WHERE id = $id";
  }else{
    $sql = "UPDATE karya SET judul = '$judul', sinopsis = '$sinopsis', tipe = '$tipe', konten = '$konten' WHERE id = $id";
  }
  ...
  if($query){
    ...
    echo '
        <script>
        alert("Gagal diedit");
        window.location.href="../action/pratinjau.php?id='.$id.'"
        </script>
        ';
  }}
